...
class Player(object):
	...
	def Summary(self):
		...
		print(self.stats)
	def Attack(self):
		x = twenty.Roll() + (self.stats['Strength']-10)/2//1
		return (x)
	def Dmg(self):
		x = weapon_info(self.weapon,Weapons)
		if self.weapon == 'Fist':
...
#player skill checks 
	...
	return(int(check))
def monster_check(monster):
#stat must be string (dex,st,con,ect)
#monster skill checks currently broken
	mod = ((monster.dex-10)/2)//1
	x = twenty.Roll()
	check = x + mod 
	return(int(check))
